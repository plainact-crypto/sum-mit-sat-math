...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'expanding-expressions-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing expanding-expressions-explanation.html');
const route='/advanced-math/equivalent-expressions/expanding-expressions/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Expanding Expressions</div>','lesson-content','EXPLANATION_CLASSIFICATION: NEITHER','3(x + 4) - 2(x - 5)','3[2x - (x - 4)]'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed>1)throw new Error(`Expected at most one scheduled route for Expanding Expressions Explanation; removed ${removed}`);
 if(removed===1){
