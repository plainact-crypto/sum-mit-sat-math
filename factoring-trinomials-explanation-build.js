...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'factoring-trinomials-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing factoring-trinomials-explanation.html');
const route='/advanced-math/factoring/factoring-trinomials/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Factoring Trinomials</div>','lesson-content','EXPLANATION_CLASSIFICATION: DESMOS_USEFUL','x² + 9x + 20','6x² + 11x + 3','x²+7x+12'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed>1)throw new Error(`Expected at most one scheduled route for Factoring Trinomials Explanation; removed ${removed}`);
 if(removed===1){
