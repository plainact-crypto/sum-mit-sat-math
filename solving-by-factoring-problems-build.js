...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'solving-by-factoring-problems.html');
const route='/advanced-math/quadratic-equations/solving-by-factoring/problems/';
if(!fs.existsSync(source))throw new Error('Missing Solving by Factoring problems source');
const html=fs.readFileSync(source,'utf8');
for(const marker of ['Solving by Factoring','SKILL CHECK','CORE PRACTICE','EXAM-STYLE PRACTICE','CHALLENGE PROBLEMS','3/8/5/2 required'])if(!html.includes(marker))throw new Error('Problems marker missing: '+marker);
const out=path.join(dist,route.replace(/^\//,''));
...
console.log('Built Solving by Factoring problems route and completed its schedule entry');
