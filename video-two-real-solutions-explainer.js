...
const route=path.join(dist,'advanced-math','quadratic-equations','two-real-solutions','video','english','index.html');
if(!fs.existsSync(route)) throw new Error('Two Real Solutions English video route missing');
let html=fs.readFileSync(route,'utf8');
const marker='SUMMIT_EXPLAINER:two-real-solutions';
if(!html.includes(marker)){
 ...
 if(!html.includes(oldBody)) throw new Error('Two Real Solutions video placeholder not found');
 ...
 const url='https://scrimba.com/explain/guide0llfa4mc4?claim=5k2k8fpmjojuvgok&fullscreen=1';
 const body='<div class="summit-video-shell summit-explainer-player" data-desmos-aligned="true"><!-- '+marker+' --><div class="summit-explainer-frame"><iframe src="'+url+'" title="Two Real Solutions — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="'+url+'" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Discriminant · Two x-intercepts · Parameter conditions · SAT method · Quick check</p></div>'+style;
 html=html.replace(oldBody,body);fs.writeFileSync(route,html);
}
console.log('Embedded Two Real Solutions English explainer');
