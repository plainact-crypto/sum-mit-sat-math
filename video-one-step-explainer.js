...
const nativeBody = /<div class="summit-video-shell"><div class="summit-native-player"[\s\S]*?<\/script>/;
const lesson = 'Solving One-Step Linear Equations';
const slug = 'solving-one-step-linear-equations';
const url = 'https://scrimba.com/explain/guide0r0g68ln3?claim=mcf83nb4tnm3gr00&fullscreen=1';
...
if (!html.includes(tag)) {
  ...
  const body = `<div class="summit-video-shell summit-explainer-player" data-graph-aligned="false" data-desmos-aligned="true" data-requested-voice-gender="male"><!-- ${tag} --><div class="summit-explainer-frame"><iframe src="${url}" title="${lesson} — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="${url}" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Teacher-style explanation · Inverse operations · Worked examples · Real-life example · Desmos verification · Quick check</p></div>${style}`;
  if (html.includes(oldBody)) html = html.replace(oldBody, body);
  else if (nativeBody.test(html)) html = html.replace(nativeBody, body);
  else throw new Error(`No replaceable English video body found for ${lesson}`);
