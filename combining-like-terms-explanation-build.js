...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'combining-like-terms-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing combining-like-terms-explanation.html');
const route='/advanced-math/equivalent-expressions/combining-like-terms/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Combining Like Terms</div>','lesson-content','3x + 5x - 2','6x² + 2x + 3'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed!==1)throw new Error(`Expected exactly one scheduled route for Combining Like Terms Explanation; removed ${removed}`);
