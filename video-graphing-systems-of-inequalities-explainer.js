...
function embed({lesson, slug, url, note, attrs=''}) {
  const hits = walk(dist).filter(file => {
    const normalized = file.split(path.sep).join('/');
    const html = fs.readFileSync(file, 'utf8');
    return normalized.endsWith(`algebra/systems-of-linear-inequalities/${slug}/video/english/index.html`) && html.includes(`<div class="lesson-title">${lesson}</div>`) && html.includes(marker);
  });
...
embed({
  lesson: 'Graphing Systems of Inequalities',
  slug: 'graphing-systems-of-inequalities',
  url: 'https://scrimba.com/explain/guide089kp5qfm?claim=pmlcjuaq62qi208j&fullscreen=1',
  ...
  note: 'English · Teacher-style explanation · Verified two-line graph and overlap · Solid/dashed boundaries · Desmos strategy · Quick check'
});

embed({
  lesson: 'Testing Points in a System',
  slug: 'testing-points-in-a-system',
  url: 'https://scrimba.com/explain/guide0p6uib10t?claim=f4di703vs8bc8sq9&fullscreen=1',
