...
const lesson = 'Graphing Systems of Inequalities';
const slug = 'graphing-systems-of-inequalities';
const routeSlug = 'graphing-systems-of-inequalities';
const url = 'https://scrimba.com/explain/guide089kp5qfm?claim=pmlcjuaq62qi208j&fullscreen=1';
...
const hits = walk(dist).filter(file => {
  ...
  return normalized.endsWith(`algebra/systems-of-linear-inequalities/${routeSlug}/video/english/index.html`) && html.includes(`<div class="lesson-title">${lesson}</div>`) && html.includes(marker);
});
...
if (!html.includes(tag)) {
  ...
  const body = `<div class="summit-video-shell summit-explainer-player" data-graph-aligned="true" data-desmos-aligned="true" data-requested-voice-gender="male" data-voice-gender-status="unavailable"><!-- ${tag} --><div class="summit-explainer-frame"><iframe src="${url}" title="${lesson} — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="${url}" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Teacher-style explanation · Verified two-line graph and overlap · Solid/dashed boundaries · Desmos strategy · Quick check</p></div>${style}`;
