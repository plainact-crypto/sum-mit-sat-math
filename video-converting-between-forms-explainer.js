...
const lesson = 'Converting Between Forms';
const slug = 'converting-between-forms';
const routeSlug = 'converting-between-forms';
const url = 'https://scrimba.com/explain/guide0a2k81o7b?claim=87l2k7f4ncphjlmh&fullscreen=1';
...
const hits = walk(dist).filter(file => {
  ...
  return normalized.endsWith(`algebra/linear-equations-in-two-variables/${routeSlug}/video/english/index.html`) && html.includes(`<div class="lesson-title">${lesson}</div>`) && html.includes(marker);
});
...
if (!html.includes(tag)) {
  ...
  const body = `<div class="summit-video-shell summit-explainer-player" data-graph-aligned="true" data-desmos-aligned="true" data-requested-voice-gender="unavailable" data-voice-gender-status="unavailable"><!-- ${tag} --><div class="summit-explainer-frame"><iframe src="${url}" title="${lesson} — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="${url}" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Teacher-style explanation · Verified equivalent forms and graph points · Desmos equivalence check · Quick check</p></div>${style}`;
