...
const allowedTitles=new Set(registry.map(sourceTitle));for(const p of published){const m=p.html.match(/<div class="lesson-title">([^<]+)<\/div>/);const title=m&&m[1];if(title&&!allowedTitles.has(title))fail(`Published Explanation is outside classification registry: ${title}`)}
