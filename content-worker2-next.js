...
const route='algebra/systems-of-linear-equations/one-solution/explanation/';
const target=path.join(dist,route);fs.mkdirSync(target,{recursive:true});fs.copyFileSync(path.join(root,'one-solution-explanation.html'),path.join(target,'index.html'));
...
if(fs.existsSync(schedulePath)){
  ...
  const schedule=original.filter(r=>r.route!==completedRoute),removed=original.length-schedule.length;
