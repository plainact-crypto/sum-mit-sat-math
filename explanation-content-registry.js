...
module.exports=[
  ...
  {
    slug:'graphing-systems-of-inequalities',lesson:'Graphing Systems of Inequalities',classification:'BOTH',
    graph:{bounds:{xMin:-2,xMax:6,yMin:-4,yMax:8},lines:[{m:1,b:-1,qaX:[-1,0,3],yIntercept:-1,xIntercept:1},{m:-1,b:5,qaX:[-1,0,3],yIntercept:5,xIntercept:5}],intersections:[{a:0,b:1,x:3,y:2}],points:[{x:3,y:2,line:0,label:'boundary intersection (3, 2)'},{x:0,y:0,label:'overlap solution (0, 0)'}],ariaLabel:'Boundary lines y equals x minus 1 and y equals negative x plus 5 intersecting at (3, 2), with verified overlap solution (0, 0)'},
    desmos:D('Enter y >= x - 1 and y <= -x + 5 on separate lines.','the overlap where both shaded regions are true, bounded by lines meeting at (3, 2)','identify the common shaded region as the solution set and verify included boundary points such as (3, 2)','a system solution must satisfy every inequality at the same time, so only the overlap belongs to the system','For a graphing task, Desmos is often the fastest accurate check; by hand, graph each boundary and test a point so you still understand why the overlap is correct.','Set the boundaries equal: x-1=-x+5 gives x=3 and y=2. Boundary QA: y=x-1 gives -2,-1,2 at x=-1,0,3; y=-x+5 gives 6,5,2. Also (0,0) satisfies 0>=-1 and 0<=5, while (4,0) fails 0>=3.')
  }
];
