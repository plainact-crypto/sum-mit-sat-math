...
const source=path.join(root,'no-real-solutions-explanation.html');
const route='/advanced-math/quadratic-equations/no-real-solutions/explanation/';
if(!fs.existsSync(source))throw new Error('Missing No Real Solutions explanation source');
const html=fs.readFileSync(source,'utf8');
for(const marker of ['No Real Solutions','EXPLANATION_CLASSIFICATION: DESMOS_USEFUL','DESMOS STRATEGY','Enter','Look for','Use it to answer','Why it works','Faster or not?','Math cross-check'])if(!html.includes(marker))throw new Error('No Real Solutions explanation marker missing: '+marker);
...
console.log('Built No Real Solutions explanation route');
