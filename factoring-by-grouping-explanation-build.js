...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'factoring-by-grouping-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing factoring-by-grouping-explanation.html');
const route='/advanced-math/factoring/factoring-by-grouping/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Factoring by Grouping</div>','lesson-content','EXPLANATION_CLASSIFICATION: DESMOS_USEFUL','x³+x²−4x−4','DESMOS STRATEGY','y=x^3+x^2-4x-4','x = -2','x = -1','x = 2'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed>1)throw new Error(`Expected at most one scheduled route for Factoring by Grouping Explanation; removed ${removed}`);
 if(removed===1){
...
console.log('Built Factoring by Grouping explanation route');
