...
const lesson = 'Slope from a Table';
const slug = 'slope-from-a-table';
const url = 'https://scrimba.com/explain/guide0pm04agug?claim=qqmsp1uehgg58shh&fullscreen=1';
...
const hits = walk(dist).filter(file => {
  ...
  return normalized.endsWith('algebra/linear-functions/slope-from-a-table/video/english/index.html') && html.includes(`<div class="lesson-title">${lesson}</div>`) && html.includes(marker);
});
