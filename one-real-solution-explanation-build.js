...
const source=path.join(root,'one-real-solution-explanation.html');
const route='/advanced-math/quadratic-equations/one-real-solution/explanation/';
if(!fs.existsSync(source))throw new Error('Missing One Real Solution explanation source');
const html=fs.readFileSync(source,'utf8');
for(const marker of ['One Real Solution','EXPLANATION_CLASSIFICATION: DESMOS_USEFUL','DESMOS STRATEGY','Enter','Look for','Use it to answer','Why it works','Faster or not?','Math cross-check'])if(!html.includes(marker))throw new Error('One Real Solution explanation marker missing: '+marker);
for(const check of ['D=0','x²−8x+16=(x−4)²','At x=4, 4²−8(4)+16=16−32+16=0'])if(!html.includes(check))throw new Error('One Real Solution math cross-check missing: '+check);
...
console.log('Built One Real Solution explanation route');
