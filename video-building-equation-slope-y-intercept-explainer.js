...
const lesson = 'Building an Equation from Slope and Y-Intercept';
const slug = 'building-equation-slope-y-intercept';
const routeSlug = 'building-an-equation-from-slope-and-y-intercept';
const url = 'https://scrimba.com/explain/guide00f0op7bu?claim=r8e025le5ek8cgr8&fullscreen=1';
...
const hits = walk(dist).filter(file => {
  ...
  return normalized.endsWith(`algebra/linear-functions/${routeSlug}/video/english/index.html`) && html.includes(`<div class="lesson-title">${lesson}</div>`) && html.includes(marker);
});
