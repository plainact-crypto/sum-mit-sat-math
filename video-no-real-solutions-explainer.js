...
const route=path.join(dist,'advanced-math','quadratic-equations','no-real-solutions','video','english','index.html');
if(!fs.existsSync(route)) throw new Error('No Real Solutions English video route missing');
let html=fs.readFileSync(route,'utf8');
const marker='SUMMIT_EXPLAINER:no-real-solutions';
if(!html.includes(marker)){
 ...
 if(!html.includes(oldBody)) throw new Error('No Real Solutions video placeholder not found');
 ...
 const url='https://scrimba.com/explain/guide0obrrsq9v?claim=7cbgoqkantctsm3b&fullscreen=1';
 const body='<div class="summit-video-shell summit-explainer-player" data-desmos-aligned="true"><!-- '+marker+' --><div class="summit-explainer-frame"><iframe src="'+url+'" title="No Real Solutions — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="'+url+'" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Discriminant · Parameter conditions · Graph meaning · SAT method · Quick check</p></div>'+style;
 html=html.replace(oldBody,body);fs.writeFileSync(route,html);
}
console.log('Embedded No Real Solutions English explainer');
