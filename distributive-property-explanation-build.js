...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'distributive-property-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing distributive-property-explanation.html');
const route='/advanced-math/equivalent-expressions/distributive-property/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Distributive Property</div>','lesson-content','4(x + 3)','-3(2y - 5)','5(3x+4)'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed!==1)throw new Error(`Expected exactly one scheduled route for Distributive Property Explanation; removed ${removed}`);
