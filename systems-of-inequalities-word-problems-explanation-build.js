...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'systems-of-inequalities-word-problems-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing systems-of-inequalities-word-problems-explanation.html');
const route='/algebra/systems-of-linear-inequalities/systems-of-inequalities-word-problems/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Systems of Inequalities Word Problems</div>','lesson-content','x + y ≤ 10','x + 2y ≥ 12'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
