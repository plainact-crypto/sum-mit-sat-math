...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'solving-by-factoring-answers.html');
const route='/advanced-math/quadratic-equations/solving-by-factoring/answers/';
if(!fs.existsSync(source))throw new Error('Missing Solving by Factoring answers source');
const html=fs.readFileSync(source,'utf8');
if(!html.includes('Solutions correspond exactly to Practice Questions 1–18.')||(html.match(/class="answer"/g)||[]).length!==18)throw new Error('Answers source must contain exactly 18 solutions');
...
require('./solving-by-factoring-test-build.js');
console.log('Built Solving by Factoring answers and lesson test routes');
