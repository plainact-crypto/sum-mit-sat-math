...
const route=path.join(dist,'advanced-math','quadratic-equations','product-of-roots','video','english','index.html');
if(!fs.existsSync(route)) throw new Error('Product of Roots English video route missing');
let html=fs.readFileSync(route,'utf8');
const marker='SUMMIT_EXPLAINER:product-of-roots';
if(!html.includes(marker)){
  ...
  if(!html.includes(oldBody)) throw new Error('Product of Roots video placeholder not found');
  ...
  const body='<div class="summit-video-shell summit-explainer-player" data-desmos-aligned="true"><!-- '+marker+' --><div class="summit-explainer-frame"><iframe src="https://scrimba.com/explain/guide079ujj7ct?claim=kc0jb23g8tfq4rqb&fullscreen=1" title="Product of Roots — SUMMIT MATH video lesson" loading="eager" allow="autoplay; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe></div><div class="summit-explainer-actions"><a href="https://scrimba.com/explain/guide079ujj7ct?claim=kc0jb23g8tfq4rqb&fullscreen=1" target="_blank" rel="noopener">Open full-screen lesson ↗</a></div><p class="video-note">English · Coefficient rule c/a · Worked examples · Desmos verification · Quick check</p></div>'+style;
  html=html.replace(oldBody,body);
  fs.writeFileSync(route,html);
}
console.log('Embedded Product of Roots English explainer');