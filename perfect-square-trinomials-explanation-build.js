...
const root=__dirname,dist=path.join(root,'dist'),source=path.join(root,'perfect-square-trinomials-explanation.html');
if(!fs.existsSync(source))throw new Error('Missing perfect-square-trinomials-explanation.html');
const route='/advanced-math/factoring/perfect-square-trinomials/explanation/';
const html=fs.readFileSync(source,'utf8');
for(const marker of ['<div class="page-type">Explanation</div>','<div class="lesson-title">Perfect-Square Trinomials</div>','lesson-content','EXPLANATION_CLASSIFICATION: DESMOS_USEFUL','a² + 2ab + b² = (a+b)²','DESMOS STRATEGY','y=x²−10x+25'])if(!html.includes(marker))throw new Error(`Explanation source failed marker: ${marker}`);
...
if(fs.existsSync(sp)){
 ...
 if(removed>1)throw new Error(`Expected at most one scheduled route for Perfect-Square Trinomials Explanation; removed ${removed}`);
 if(removed===1){
